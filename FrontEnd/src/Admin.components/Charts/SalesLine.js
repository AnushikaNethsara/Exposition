...
class SalesLine extends Component{
    ...
    constructor(props){
        ...
        this.getentrybyDate = this.getentrybyDate.bind(this);
   ...
   async getAllDates() {
      ...
      await  axios.get(`http://${config.host}:${config.port}/registration/`, {
          headers:
          {
            token: token
     ...
     async getentrybyDate(){
        ...
        for (let index = 0; index < this.state.DistinctDatesArray.length; index++) {
          ...
          //set label
           ...
           const token = localStorage.getItem('token');
     await   axios.get(`http://${config.host}:${config.port}/registration/byDate/` + key, {
            headers:
            {
              token: token
